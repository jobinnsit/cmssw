 ...
 *  cm.
 *  It uses the input file extractd from cmssw clustering on GPU
 *
 */
  ...
  for(int e=1;e<=100;e++) { 
     ...
     Index[total_cluster]= i;
      ...
      for(auto itm = ClusterInfo.begin(); itm!=ClusterInfo.end(); itm++) {  // for each module
        ...
        //if(clustId!=416046)  continue;
        uint moduleId = clustId/1000000; // to get the moduleId divide by 10^6
...
// old main 
/*
this main reads input file generated from cmssw
int main(int argc, char const *argv[]) {
 ...
 for (auto itm = theModule.begin(); itm!=theModule.end(); itm++) {  // for each module
     ...
     for (auto itc=clusVec.begin(); itc!=clusVec.end();itc++) {  // for each cluster
      ...
      auto pixVec = *itc;
      LocalPoint lp = pixelHit(detDB,moduleId, pixVec);
      cout<<setw(14)<<moduleId<<setw(10)<<clusterId++<<setw(20)<<lp.x()<<setw(20)<<lp.y()<<endl;
      //cout<<lp.x()<<"    "<<lp.y()<<endl; 
     }
  }
