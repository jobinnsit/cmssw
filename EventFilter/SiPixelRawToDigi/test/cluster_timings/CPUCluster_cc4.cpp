...
int main(int argc, char **argv) {
  ...
  CPUTIME.open("Cluster_CPU_Time.txt");
  CPUTIME<<"Event\t Total pixel\t Time(us)"<<endl;
  ...
  for(int Event=1;Event<101;Event++) {
    ...
    i=0;
    uint mId, start , end;
    ifile.open("InputForCPU_Cluster/ModuleStartEndIndex_evt"+to_string(Event)+".txt");
    getline(ifile, str);
    while(!ifile.eof()) {
      ifile>>mId>>start>>end;
      mIndexEnd[i] = end;
    ...
    for(int ml=0;ml<total_module;ml++) {          // for each module form the cluster
      ...
      // skip the bad madule and empty module
      ...
      // if module contains only on pixel
        ...
        outputFile <<setw(12)<<gClusterId<<setw(6)<<yy[moduleBegin]<<setw(6)<<xx[moduleBegin]<<endl;
        continue;
	  }
  ...
  double mic_seconds = duration_cast<microseconds>(t2-t1).count();
  CPUTIME<<setw(4)<<Event<<setw(8)<<wordCounter<<setw(10)<<mic_seconds<<endl;
