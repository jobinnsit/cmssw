void rtdplot_GPU() {
   ...
   fp  = fopen("R2D_GPU_Time.txt", "r");
   Double_t event =0,xc=0, yc=0;
   Int_t i =0;
   char line[100];
   fgets(line, sizeof line, fp);
   
   // skip the first entry as it involes the launching overhead
   fscanf(fp,"%lf %lf %lf", &event, &xc, &yc);
   while(!feof(fp)){
   ...
   gr->SetTitle("R2D GPU Time vs Number of Pixels");
